(function () {
    ...
    ***************************************************************************** */
    ...
    var QueryGenerator = /** @class */ (function () {
        ...
        QueryGenerator.prototype.toString = function (main) {
            ...
            for (var key in this.queryObject) {
                ...
                if (key == 'expand') {
                    prefix.push("$expand=" + this.queryObject.expand.map(function (queryObject) {
    ...
    var STAInterface = /** @class */ (function () {
        function STAInterface(baseUrl) {
            this.baseUrl = baseUrl;
        }
        STAInterface.prototype.getGeoJson = function (query) {
            var _this = this;
            return new Promise(function (resolve, reject) { return __awaiter(_this, void 0, void 0, function () {
                var url, data, link, response;
                var _a;
                return __generator(this, function (_b) {
                    switch (_b.label) {
                        case 0:
                            url = this.baseUrl + "/" + (new QueryGenerator(query).toString());
                        ...
                        case 5:
                            response = _b.sent();
                            (_a = data.value).push.apply(_a, response.value);
    ...
    var poly = index;
    var MapInterface = /** @class */ (function () {
        function MapInterface(config) {
            this.loadedQuads = {};
            this.cache = {};
            this.config = config;
            this.api = new STAInterface(config.baseUrl);
        }
        MapInterface.prototype.long2tile = function (lon, zoom) { return (Math.floor((lon + 180) / 360 * Math.pow(2, zoom))); };
        MapInterface.prototype.lat2tile = function (lat, zoom) { return (Math.floor((1 - Math.log(Math.tan(lat * Math.PI / 180) + 1 / Math.cos(lat * Math.PI / 180)) / Math.PI) / 2 * Math.pow(2, zoom))); };
        MapInterface.prototype.tile2long = function (x, z) {
            return (x / Math.pow(2, z) * 360 - 180);
        };
        MapInterface.prototype.tile2lat = function (y, z) {
            var n = Math.PI - 2 * Math.PI * y / Math.pow(2, z);
            return (180 / Math.PI * Math.atan(0.5 * (Math.exp(n) - Math.exp(-n))));
        };
        MapInterface.prototype.coordinatesToOsm = function (coordinate, zoom) {
            var x = this.tile2long(this.long2tile(coordinate.x, zoom), zoom);
            var y = this.tile2lat(this.lat2tile(coordinate.y, zoom), zoom);
            return { x: x, y: y };
        };
        MapInterface.prototype.coordinatesToOsmBottom = function (coordinate, zoom) {
            var x = this.tile2long(this.long2tile(coordinate.x, zoom) + 1, zoom);
            var y = this.tile2lat(this.lat2tile(coordinate.y, zoom) + 1, zoom);
            return { x: x, y: y };
        };
        MapInterface.prototype.getOSMBoundingBox = function (zoomLevel, boundingBox) {
            var topleft = { x: 0, y: 0 };
        ...
        MapInterface.prototype.getLayerData = function (zoom, boundingBox) {
            return __awaiter(this, void 0, void 0, function () {
                var correctedQuery, osmbb, topx, topy, bottomx, bottomy, geo, poly1, data, locations, osmQuads, toMarker, combine, markerQuery, markers;
                var _a;
                var _this = this;
                return __generator(this, function (_b) {
                    switch (_b.label) {
                        case 0:
                            correctedQuery = JSON.parse(JSON.stringify(this.config.queryObject));
                            correctedQuery.select = ['id'];
                            correctedQuery.expand = [{ entityType: "Locations" }];
                            if (correctedQuery.entityType == 'Things') {
                                osmbb = this.getOSMBoundingBox(zoom, boundingBox);
                                topx = osmbb[0];
                                topy = osmbb[1];
                                bottomx = osmbb[2];
                                bottomy = osmbb[3];
                                if (this.loadedQuads[zoom]) {
                                    poly1 = [[[topx, topy], [topx, bottomy], [bottomx, bottomy], [bottomx, topy], [topx, topy]]];
                                    this.loadedQuads[zoom].forEach(function (element) {
                                        poly1 = poly.difference(poly1, [element]);
                                    });
                                    if (poly1[0])
                                        geo = "geo.intersects(Locations/location,geography'POLYGON ((" + poly1[0][0].map(function (el) { return el.join(' '); }).join(',') + ")) ')";
                                    else
                                        geo = 'FALSE';
                                }
                                else {
                            ...
                            return [4 /*yield*/, this.api.getGeoJson(correctedQuery)];
                        case 1:
                            data = _b.sent();
                            locations = data.value.map(function (data) { return data.Locations[0].location; });
                            osmQuads = [];
                            locations = locations.map(function (location) {
                                var top = _this.coordinatesToOsm({ x: location.coordinates[0], y: location.coordinates[1] }, zoom);
                                var bottom = _this.coordinatesToOsmBottom({ x: location.coordinates[0], y: location.coordinates[1] }, zoom);
                                var obj = {
                                    "type": "Feature",
                                    "geometry": {
                                        "type": "Polygon",
                            ...
                            locations.forEach(function (element) {
                                var existing = osmQuads.find(function (geo) {
                                ...
                                if (!existing) {
                                    ...
                                    if (!_this.loadedQuads[zoom])
                                        _this.loadedQuads[zoom] = [];
                                    _this.loadedQuads[zoom].push(element.geometry.coordinates);
                                }
                                else {
                            ...
                            toMarker = [];
                            osmQuads = osmQuads.filter(function (geo) {
                                //TODO use config
                            ...
                            toMarker.forEach(function (elem, index) {
                                if (index == 0)
                                    return combine = elem.geometry.coordinates;
                                combine = poly.union(combine, [elem.geometry.coordinates]);
                            });
                            if (!combine) return [3 /*break*/, 3];
                            markerQuery = JSON.parse(JSON.stringify(this.config.queryObject));
                            if (!markerQuery.expand)
                                markerQuery.expand = [];
                            if (!markerQuery.expand.includes(function (expand) { return expand.entityType == 'Location'; })) {
                                markerQuery.expand.push({ entityType: "Locations" });
                            }
                            if (markerQuery.filter)
                                markerQuery.filter = "(" + markerQuery.filter + ") and ";
                            markerQuery.filter = markerQuery.filter ? markerQuery.filter : '' + combine.map(function (e) {
                                return "geo.intersects(Locations/location,geography'POLYGON ((" + (e[0][0][0] ? e[0] : e).map(function (array) { return array.join(' '); }).join(',') + "))')";
                            }).join(' or ');
                            return [4 /*yield*/, this.api.getGeoJson(markerQuery)];
                        case 2:
                            markers = _b.sent();
                        ...
                        case 3:
                            if (this.cache[zoom])
                                (_a = this.cache[zoom].features).push.apply(_a, osmQuads);
                            else
                                this.cache[zoom] = {
                                    'type': 'FeatureCollection',
                                    'features': osmQuads
                                };
                            return [2 /*return*/, this.cache[zoom]];
                    }
                });
            });
        };
                ...
                this.on('add', function () {
                    if (this._map != undefined) {
                        ...
                        map.on('layeradd', function () {
                            ...
                            mapInterface.getLayerData(map.getZoom(), [bounds._northEast.lng, bounds._northEast.lat, bounds._southWest.lng, bounds._southWest.lat]).then(function (data) {
